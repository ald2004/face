<template>
  <div class="head-container">
    ...
    <div style="display: inline-block;margin: 0px 2px;">
      <el-button
        v-if="checkPermission(['ADMIN','ACCESSCONTROL_ALL','ACCESSCONTROL_CREATE'])"
        class="filter-item"
        size="mini"
        type="primary"
        icon="el-icon-plus"
  ...
  methods: {
    ...
    download() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        const tHeader = ['姓名', '性别', '身份证号码', '住址', '人员类型', '年龄段', '创建时间'];
        const filterVal = ['username', 'sex', 'idCard', 'address','type','oldTime', 'createTime'];
