...
VlHog *vl_hog_new(VlHogVariant variant, vl_size numOrientations, vl_bool transposed) {
    ...
    self->orientationX = (float *) malloc((size_t) (sizeof(float) * self->numOrientations));
    self->orientationY = (float *) malloc((size_t) (sizeof(float) * self->numOrientations));
    ...
    for (o = 0; o < (signed) self->numOrientations; ++o) {
        float angle = (float) (o * VL_PI / self->numOrientations);
        if (!self->transposed) {
            self->orientationX[o] = cos(angle);
            self->orientationY[o] = sin(angle);
        } else {
            self->orientationX[o] = sin(angle);
            self->orientationY[o] = cos(angle);
        }
    }
    ...
    switch (self->variant) {
        case VlHogVariantUoctti:
            for (o = 0; o < (signed) self->numOrientations; ++o) {
                ...
                self->permutation[o + self->numOrientations] =
                        (op + self->numOrientations) % (2 * self->numOrientations);
                self->permutation[o + 2 * self->numOrientations] =
                        (op % self->numOrientations) + 2 * self->numOrientations;
            }
            for (k = 0; k < 4; ++k) {
        ...
        case VlHogVariantDalalTriggs:
            for (k = 0; k < 4; ++k) {
                ...
                for (o = 0; o < (signed) self->numOrientations; ++o) {
                    vl_index op = self->numOrientations - o;
                    self->permutation[o + k * self->numOrientations] =
                            (op % self->numOrientations) + q * self->numOrientations;
                }
            }
            break;
