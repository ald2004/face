...
cv::Mat
CalculateHogDescriptor(cv::Mat &src, cv::Mat shape, std::vector<int> LandmarkIndexs, std::vector<int> eyes_index,
                       HoGParam mHoGParam) {
...
//    cv::waitKey(30);
    ...
    float ly =
            (shape.at<float>(eyes_index.at(0) + numLandmarks) + shape.at<float>(eyes_index.at(1) + numLandmarks)) * 0.5;
    float rx = (shape.at<float>(eyes_index.at(2)) + shape.at<float>(eyes_index.at(3))) * 0.5;
    float ry =
            (shape.at<float>(eyes_index.at(2) + numLandmarks) + shape.at<float>(eyes_index.at(3) + numLandmarks)) * 0.5;
    ...
    for (int i = 0; i < LandmarkIndexs.size(); i++) {
        ...
        cv::Mat roiImg;
        if (x - patch_width_half < 0 || y - patch_width_half < 0 || x + patch_width_half >= grayImage.cols ||
            y + patch_width_half >= grayImage.rows) {
            ...
            int borderLeft =
                    (x - patch_width_half) < 0 ? std::abs(x - patch_width_half) : 0; // x and y are patch-centers
            int borderTop = (y - patch_width_half) < 0 ? std::abs(y - patch_width_half) : 0;
            int borderRight =
                    (x + patch_width_half) >= grayImage.cols ? std::abs(grayImage.cols - (x + patch_width_half)) : 0;
            int borderBottom =
                    (y + patch_width_half) >= grayImage.rows ? std::abs(grayImage.rows - (y + patch_width_half)) : 0;
            cv::Mat extendedImage = grayImage.clone();
            cv::copyMakeBorder(extendedImage, extendedImage, borderTop, borderBottom, borderLeft, borderRight,
                               cv::BORDER_CONSTANT, cv::Scalar(0));
            cv::Rect roi((x - patch_width_half) + borderLeft, (y - patch_width_half) + borderTop, patch_width_half * 2,
                         patch_width_half * 2); // Rect: x y w h. x and y are top-left corner.
            roiImg = extendedImage(roi).clone(); // clone because we need a continuous memory block
        } else {
            cv::Rect roi(x - patch_width_half, y - patch_width_half, patch_width_half * 2, patch_width_half *
                                                                                           2); // x y w h. Rect: x and y are top-left corner. Our x and y are center. Convert.
        ...
        VlHog *hog = vl_hog_new(VlHogVariant::VlHogVariantUoctti, mHoGParam.num_bins,
                                false); // transposed (=col-major) = false
        vl_hog_put_image(hog, (float *) roiImg.data, roiImg.cols, roiImg.rows, 1,
                         mHoGParam.cell_size); // (the '1' is numChannels)
        ...
        for (int j = 0; j < dd; ++j) {
            cv::Mat hogFeatures(hh, ww, CV_32FC1, hogArray.ptr<float>(0) + j * ww *
                                                                           hh); // Creates the same array as in Matlab. I might have to check this again if hh!=ww (non-square)
