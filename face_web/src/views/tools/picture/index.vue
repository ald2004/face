    ...
    <el-table v-loading="loading" :data="data" size="small" border style="width: 100%;">
      ...
      <el-table-column :show-overflow-tooltip="true" prop="url" label="链接地址">
        <template slot-scope="scope">
          <a :href="scope.row.url.startsWith('http')?scope.row.url:baseUrl+'/'+scope.row.url " style="color: #42b983" target="_blank">{{ scope.row.url.startsWith('http')?scope.row.url:baseUrl+'/'+scope.row.url }}</a>
        </template>
      </el-table-column>
