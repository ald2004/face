...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :show-overflow-tooltip="true" prop="idCard" label="身份证"/>
      ...
      <el-table-column :show-overflow-tooltip="true" prop="region" label="安装位置"/>
      ...
      <el-table-column align="center" prop="status" width="80px" label="状态">
        <template slot-scope="scope">
          <span class="badge" v-if="scope.row.status === 0">禁用</span>
          <span class="badge" v-if="scope.row.status === 1">黑名单</span>
          <span class="badge badge-bg-green" v-if="scope.row.status === 2">白名单</span>
      ...
      <el-table-column :show-overflow-tooltip="true" prop="createTime" label="过去时间">
        <template slot-scope="scope">
          <el-tag v-if="timestampFormat(scope.row.createTime).status===0">{{timestampFormat(scope.row.createTime).text}}</el-tag>
          <el-tag v-if="timestampFormat(scope.row.createTime).status===1" type="success">{{timestampFormat(scope.row.createTime).text}}</el-tag>
          <el-tag v-if="timestampFormat(scope.row.createTime).status===2" type="info">{{timestampFormat(scope.row.createTime).text}}</el-tag>
          <el-tag v-if="timestampFormat(scope.row.createTime).status===3" type="warning">{{timestampFormat(scope.row.createTime).text}}</el-tag>
          <el-tag v-if="timestampFormat(scope.row.createTime).status===4" type="danger">{{timestampFormat(scope.row.createTime).text}}</el-tag>
        </template>
      </el-table-column>
...
import imgShow from "./module/img";
